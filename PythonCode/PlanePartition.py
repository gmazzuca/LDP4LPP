...
def compute_density_row(x_grid, xi, gamma_sq, eta, theta, beta):
    """Vectorized base evaluation for a single horizontal xi slice."""
    try:
...
# --- 2D GRID EXECUTION ---
configs = [
    {'gamma_sq': 0.3,  'eta': 7.0, 'theta': 5.0, 'beta': 0.5},
    {'gamma_sq': 0.5,  'eta': 4.0, 'theta': 3.0, 'beta': 0.2},
    {'gamma_sq': 0.4,  'eta': 2.0, 'theta': 5.0, 'beta': 0.3},
    {'gamma_sq': 0.25, 'eta': 3.0, 'theta': 2.0, 'beta': 1.0}
]
...
print("==================================================")
print(f" Generating Jet Colormap Density Maps ν(λ)        ")
print("==================================================")
...
for idx, cfg in enumerate(configs):
    ...
    # Define an exponentially spaced x_grid mapping exactly to linearly spaced lambdas
    x_max_val = 0.99999
    x_min_val = np.exp(-9.0) 
    
    # geomspace provides strict exponential spacing. We map decreasing x so lambda increases from 0.
    x_grid = np.geomspace(x_max_val, x_min_val, lambda_res)
    
    # Map perfectly back to a linear lambda grid via the physical relation
    lambda_vals = -np.log(x_grid) / beta
    
    XI_grid, L_grid = np.meshgrid(xi_vals, lambda_vals)
    NU_grid = np.zeros_like(XI_grid)
    
    # Arrays to trace characteristics mapped into lambda coordinates
    lambda_b_traj = np.zeros(len(xi_vals))
    lambda_max_traj = np.zeros(len(xi_vals))
    
    for i, xi in enumerate(xi_vals):
        if i % 40 == 0:
            print(f"  -> Progress: row {i}/{len(xi_vals)}")
            
        # Compute mu(x) on the exponentially spaced x-grid
        mu_vals, p = compute_density_row(x_grid, xi, gamma_sq, eta, theta, beta)
        
        # Exact Change of Variables: nu(lambda) = beta * exp(-beta*lambda) * mu(exp(-beta*lambda))
        nu_vals = beta * x_grid * mu_vals
        
        # 1. Any density exactly zero becomes NaN (so the jet map treats it as transparent/blank)
        nu_vals[nu_vals == 0.0] = np.nan
        
        # 2. Cap any density strictly > 5.0 to exactly 5.0 to saturate the scale
        nu_vals = np.where((nu_vals > 5.0) & (~np.isnan(nu_vals)), 5.0, nu_vals)
        
        NU_grid[:, i] = nu_vals
        
        # Track the edges in lambda coordinates
        if p is not None:
            x_b = max(1e-12, p['b']) ** (1.0 / p['pref_pow'])
            x_max_val_traj = max(1e-12, p['x_max']) ** (1.0 / p['pref_pow'])
            
            lambda_b_traj[i] = -np.log(x_b) / beta
            lambda_max_traj[i] = -np.log(x_max_val_traj) / beta
        else:
            lambda_b_traj[i] = np.nan
            lambda_max_traj[i] = np.nan

    # Plotting
    ax = axs[idx]
    
    # Fetch Jet colormap and explicitly tell it to render NaN values completely transparent/blank
    cmap = plt.get_cmap('jet').copy()
    cmap.set_bad(color='white', alpha=0)
    
    # Skew the scale heavily towards low values so 0.1-0.5 is brightly visible
    norm = mcolors.PowerNorm(gamma=0.4, vmin=0, vmax=5)
    
    mesh = ax.pcolormesh(XI_grid, L_grid, NU_grid, cmap=cmap, norm=norm, shading='auto')
    
    # Overlay regime boundary trajectories
    ax.plot(xi_vals, lambda_b_traj, color='#FF1493', linewidth=2.5, label=r'Outer Edge $\lambda_b$')
    ax.plot(xi_vals, lambda_max_traj, color='cyan', linewidth=2.5, linestyle=':', label=r'Constraint $\lambda_{\min}$')

    # Overlay Phase transitions
    ax.axvline(x=boundary, color='red', linestyle='--', alpha=0.8, label=r'Transition')
    ...
    ax.set_ylabel(r'$\lambda$', rotation=0, fontsize=20)
    ax.legend(loc='upper right', fontsize=18, framealpha=0.9)
    
    # 3. Dedicated colorbar assigned exactly to each subplot
    cbar = fig.colorbar(mesh, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_ticks([0, 0.1, 0.5, 1, 2, 3, 4, 5])
    cbar.set_ticklabels(['0', '0.1', '0.5', '1', '2', '3', '4', '$\geq$ 5'])
    cbar.set_label(r'$\nu(\lambda)$', rotation=0, labelpad=20, fontsize=16)
